<div class="realm-cuadro">
	...
	<?php
	$sql = mysql_query ("SELECT * FROM $server_adb.`uptime` WHERE `realmid` = 3 ORDER BY `starttime` DESC LIMIT 1");  
	...
	<div class="sidebar-module" id="sidebar">
    	...
    	<?php echo $Ind['Ind6']; ?><span class="date"><?php echo $website_3['version']; ?></span></td>
        <td width="80" align="left">
        <?php echo $Status['Tipe']; ?><span class="date"><?php echo $TypeServ_3; ?></span></td>
    	  </tr>
        <tr><td height="18">
        <?php echo $Status['PjCreat']; ?>
		<?php
		$connection_setup = mysql_connect($serveraddress . ':' . $serverport,$serveruser,$serverpass)or die(mysql_error());
		mysql_select_db($server_cdb_3, $connection_setup) or die(mysql_error());
        ...
        <?php echo $Status['Drop']; ?><span class="date"><?php echo $DropServ_3; ?></span><br />
        </td></tr>
        <tr><td height="18">
		...
		<center>
			...
			function getPlayers3($server_cdb_3,$connection_setup) {
				$db = @mysql_select_db($server_cdb_3,$connection_setup);
				if(!$db) {
					die(DFAIL);
				}
			...
			function doFaction3($server_cdb_3,$connection_setup,$a) {
				$db = @mysql_select_db($server_cdb_3,$connection_setup);
				if(!$db) {
					die(DFAIL);
				}
			...
			$p = @getPlayers($server_cdb_3,$connection_setup);
			$a = @doFaction($server_cdb_3,$connection_setup,$alliance);
			$h = @doFaction($server_cdb_3,$connection_setup,$horde);
			$ap = @percent($a,$p);
			$hp = @percent($h,$p);
			$b = @barWidth($a,$p,273);
			$c = @barWidth($h,$p,273);
			echo "<a data-tooltip='".doFaction($server_cdb_3,$connection_setup,$alliance)." <font style=\"color:#3399ff;font-weight:bold;\">".$Status['Ali']."</font> <small>".$Status['PlOnLine']."</small>'\><div style=\"width:" . $bar_width . ";height:" . $bar_height . ";\">
			...
			echo "<a data-tooltip='".doFaction($server_cdb_3,$connection_setup,$horde)." <font style=\"color:#ff3333;font-weight:bold;\">".$Status['Horde']."</font> <small>".$Status['PlOnLine']."</small>'\></div>
