...
<div class="realm-cuadro">
	<?php
	  $get_realms = mysql_query("SELECT * FROM $server_adb.realmlist WHERE `id` = 1");
	...
	<?php
	$sql = mysql_query ("SELECT * FROM $server_adb.`uptime` WHERE `realmid` = 1 ORDER BY `starttime` DESC LIMIT 1");  
	...
	elseif($uptime_results['uptime'] > 3600) $uptime =  round(($uptime_results['uptime'] / 60 / 60),2)."".$Status['Hours']."";
		...
		if (! $sock = @fsockopen($host, $world_port, $num, $error, 3))
			echo "<font color=red><b>".$Status['Uptime:']."</b></font> <span class='date'>0 ".$Status['Min']."</span> <br>";
		else
			{
	...
	<div class="sidebar-module" id="sidebar">
    	...
    	<?php echo $Ind['Ind6']; ?><span class="date"><?php echo $website['version']; ?></span></td>
        <td width="80" align="left">
        <?php echo $Status['Tipe']; ?><span class="date"><?php echo $TypeServ; ?></span></td>
    	  </tr>
        <tr><td height="18">
        ...
        <?php echo $Status['Drop']; ?><span class="date"><?php echo $DropServ; ?></span><br />
        </td></tr>
        <tr><td height="18">
        <?php echo $Status['PjConect']; ?><span class="date"><?php echo $char_on; ?></span><br />
        <td align="left">
        <?php echo $Status['Exp']; ?><span class="date"><?php echo $ExpServ; ?></span><br />
        </td>
        </tr></table>
		<span class="clear"><!-- --></span>
		<br />
		...
		<center>
			...
			$b = @barWidth($a,$p,273);
			$c = @barWidth($h,$p,273);
			echo "<a data-tooltip='".doFaction($server_cdb,$connection_setup,$alliance)." <font style=\"color:#3399ff;font-weight:bold;\">".$Status['Ali']."</font> <small>".$Status['PlOnLine']."</small>'\><div style=\"width:" . $bar_width . ";height:" . $bar_height . ";\">
			...
			echo "<a data-tooltip='".doFaction($server_cdb,$connection_setup,$horde)." <font style=\"color:#ff3333;font-weight:bold;\">".$Status['Horde']."</font> <small>".$Status['PlOnLine']."</small>'\></div>
